...
namespace data {
template <typename T>
void getData(const std::shared_ptr<odb::database> db,
             const odb::query<T> &statement, odb::result<T> &res) {
  try {
...
odb::result<closingPrice> VolStrategy::getData(std::string esteeID,
                                               btime::ptime startdate,
                                               btime::ptime enddate) {
  typedef odb::query<closingPrice> query;
  typedef odb::result<closingPrice> result;
  result res, data;
  query q(query::id.Estee_ID == esteeID && query::id.Date >= startdate &&
...
void VolStrategy::getAllData() {
  ...
  masterquery q(masterquery::UnderlyingEstee_Id == esteeId &&
                ...
                masterquery::Security_Type.in(
                    eaSecurityType::INDEXSPOT, eaSecurityType::INDEXFUTURES,
                    eaSecurityType::INDEXCALL, eaSecurityType::INDEXPUT) +
                    "ORDER BY" + masterquery::Security_Type);
  data::getData<EsteeMaster_Archive>(con->getConnection(), q, res);
  for (EsteeMaster_Archive &a : res) {
    if (((std::find(listOfExpiries.begin(), listOfExpiries.end(),
                    a.Expiry_Date) == listOfExpiries.end()) &&
  ...
  for (EsteeMaster_Archive &a : res) {
    ...
    pricequery querydata(
        pricequery::id.Estee_ID == a.getEsteeID() &&
        pricequery::id.Date >= startdate && pricequery::id.Date <= enddate &&

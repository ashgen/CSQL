...
void config::ReadConfigXML(const std::string& config) {
	...
	try{
		...
		archivelogger=spdlog::rotating_logger_mt("archive_logger", _archivelogfile, ASYNC_SIZE/1024, 3);
...
std::string config::esteeID() const{
	return _esteeID;
}
btime::ptime config::startdate() const{
	return _startdate;
}
btime::ptime config::enddate() const{
	return _enddate;
}
std::shared_ptr<spdlog::logger> config::log(){
	return logger;
}
std::shared_ptr<spdlog::logger> config::archivelog(){

...
CCFLAGS := -Wall -Wno-unknown-pragmas -pedantic -std=c++11 -g3 -I $(INCLUDE_DIR) -I $(ESTEE_EXT_DIR) -I $(ESTEE_INT_DIR) -I $(STRATEGY_COMMON_DIR)include -I $(OPTION_COMMON_DIR)include -I $(MM_COMMON_DIR)include -DDATABASE_SQLLITE -DDATABASE_MSSQL -DDATABASE_MYSQL
LDFLAGS :=   -L $(ESTEE_STRATEGY_LIB_DIR) -lEstee.OptionsMM -L $(ESTEE_LIBS_DIR) -lEstee.Strategy.Common  -L $(ESTEE_LIBS_DIR) -lEsteeAPI -lactivemq-cpp -lEstee.QuickFix -lonetick -lOtqQueryProcessor -lodb-mssql -lodb-mysql -lodb-sqlite  -lodb  -lodb-boost -lboost_date_time -lboost_system -lboost_thread -lpthread -lrt -lxml2
...
ODB_OBJ+= $(addprefix obj/,$(notdir $(SQL:.h=-odb-mysql.o)))
GEN_FILES:= $(INCLUDE_DIR)/*.hxx $(INCLUDE_DIR)/*.ixx $(SRC_DIR)/*.cxx
...
odb:
	odb --multi-database dynamic -d common -d mssql -d sqlite -d mysql --profile boost --generate-schema --generate-query  $(SQL)
	mv *.cxx $(SRC_DIR) 
	mv *.sql $(SQL_DIR) 

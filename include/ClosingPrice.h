...
class closingPrice {
 public:
  ...
  closingPrice(const closingPrice& a) {
    this->_id.Estee_ID = a._id.Estee_ID;
    this->_id.Date = a._id.Date;
  ...
  friend std::ostream& operator<<(std::ostream& out, const closingPrice& a) {
    out << a._id.Estee_ID << "," << boost::posix_time::to_simple_string(a._id.Date) << "," << a.Open_Interest << "," << a.Settlement_price << "," << a.Open_Price << "," << a.High_Price << ","
        << a.Low_Price << "," << a.Close_Price << "," << a.Volume << "," << a.Net_Traded_Val;
    return out;
  }
